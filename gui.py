...
selectFileButton = tk.Button(myapp, text="Selecione o Arquivo de Transcrição", command=lambda: selectPathToTranscription())
...
sourceLanguageMenu = tk.OptionMenu(myapp, variableSourceLanguage, *languageChoices,command=lambda x: setSourceLanguage())
...
targetLanguageMenu = tk.OptionMenu(myapp, variableTargetLanguage, *languageChoices,command=lambda x: setTargetLanguage())
targetLanguageMenu.pack()

# add optionMenu to select translation model
translationModelChoices = ["facebook/m2m100_1.2B", "facebook/wmt19-en-de", "facebook/wmt19-de-en"]
...
translationModelMenu = tk.OptionMenu(myapp, variableTranslationModel, *translationModelChoices,command=lambda x: setTranslationModel())
...
def selectPathToExport():
    ...
    print(pathToExport)
selectPathToExportButton = tk.Button(myapp, text="Selecione o Local onde salvar a Tradução", command=lambda: selectPathToExport())
selectPathToExportButton.pack()


# add button to save translated file



# start the program

tk.Button(myapp, text="Show Variables", command=lambda: print("PathToTranscription:", pathToTranscription,"\nTranslateModel:",translationModel, "\nTarget:", targetLanguage, "\nSource: ", sourceLanguage, "\n Path to Export: ", pathToExport)).pack()
...
def translate():
    ...
    translated_subs = Tools.translate(subs, source_language=sourceLanguage, target_language=targetLanguage, model=translationModel)
